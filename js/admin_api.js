...
(function (global) {
    ...
    const CloudVPNApi = {
         ...
         * Serialized through a promise queue to prevent nonce race conditions when multiple calls occur.
         ...
         */
        adminGetRequest: function (routeTemplate, adminEmail, adminPassword) {
            const self = this;
            // Chain to internal queue to guarantee sequential nonce generation & consumption
            const execute = async () => {
                const hashedPasswd = await self.encodeNonce(adminEmail, adminPassword);
            ...
            const resultPromise = self._requestQueue.then(execute, execute);
            self._requestQueue = resultPromise.catch(() => {});
